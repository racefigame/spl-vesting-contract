...
fn command_unlock_svc(
    ...
    let vesting_token_pubkey = get_associated_token_address(&vesting_pubkey, &header_state.mint_address);
...
fn command_info(
    ...
    for i in 0..schedules.len() {
        msg!("\nSCHEDULE {:?}", i);
        msg!("Release Time: {:?}", &schedules[i].release_time);
...
fn main() {
    let matches = App::new(crate_name!())
        ...
        .subcommand(SubCommand::with_name("unlock").about("Unlock a vesting contract. This will only release the schedules that have reached maturity.")
            .arg(
                Arg::with_name("seed")
                    .long("seed")
                    .value_name("SEED")
